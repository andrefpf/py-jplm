...
COMMAND_SHIFT_TEMPLATE = "{jplm_bin}/utils/lenslet_13x13_shifter -i {input_path}/ -o {output_path}/"
COMMAND_CONVERT_TEMPLATE = "{jplm_bin}/utils/convert_ppm_to_pgx -i {input_path} -o {output_path}/"
...
class CTCDownloader:
    ...
    def download_all_lenslets(self, path, remove_tmp=True):
        light_fields = ["Bikes", "Danger_de_Mort", "Fountain_Vincent2", "Stone_Pillars_Outside"]
        self.download_multiple_lfs(light_fields, path, remove_tmp)
    ...
    def download_multiple_lfs(self, names, target_path, remove_tmp=True):
        ...
        lenslets = ["Bikes", "Danger_de_Mort", "Fountain_Vincent2", "Stone_Pillars_Outside"]
        for name in names:
            zipped_path = Path(target_path) / f".zipped/{name}.zip"
